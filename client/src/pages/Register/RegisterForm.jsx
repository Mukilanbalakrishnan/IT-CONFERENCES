...
// --- STYLES ---
// All CSS is now included directly in the component file.
const componentStyles = `
/* --- Main Layout --- */
.main-container {
    width: 100%;
...
const ParticipantInput = ({ index, participant, handleParticipantChange, removeParticipant }) => (
    <div className="participant-block">
        <div className="participant-header">
            <h4 className="participant-title">Participant {index + 1}</h4>
            {index > 0 && (
                <button type="button" onClick={() => removeParticipant(index)} className="btn-remove">Remove</button>
            )}
        </div>
        <div className="form-grid-cols-2">
            <div>
                <label className="form-label form-label-sm">Name</label>
                <input type="text" name="name" value={participant.name} onChange={(e) => handleParticipantChange(index, e)} className="form-input-sm" required />
            </div>
            <div>
                <label className="form-label form-label-sm">Designation</label>
            ...
            <div className="participant-field-full">
                <label className="form-label form-label-sm">Organisation</label>
                <input type="text" name="organisation" value={participant.organisation} onChange={(e) => handleParticipantChange(index, e)} className="form-input-sm" required />
            </div>
            <div>
                <label className="form-label form-label-sm">Email</label>
                <input type="email" name="email" value={participant.email} onChange={(e) => handleParticipantChange(index, e)} className="form-input-sm" required />
            </div>
            <div>
                <label className="form-label form-label-sm">Phone</label>
                <input type="tel" name="phone" value={participant.phone} onChange={(e) => handleParticipantChange(index, e)} className="form-input-sm" required />
            </div>
            <div>
                <label className="form-label form-label-sm">Gender</label>
                <select name="gender" value={participant.gender} onChange={(e) => handleParticipantChange(index, e)} className="form-input-sm form-select" required>
                    <option value="" disabled>Select...</option>
                    <option value="Male">Male</option>
                    <option value="Female">Female</option>
                    <option value="Other">Other</option>
            ...
            <div>
                ...
                <input type="file" accept="image/*" name="proofFile" onChange={(e) => handleParticipantChange(index, e)} className="form-input-sm" required={!participant.proofUrl} />
                {participant.proofUrl && (
                    <div style={{ marginTop: '0.5rem' }}>
                        <img src={participant.proofUrl} alt="Proof Preview" style={{ width: '8rem', height: '8rem', objectFit: 'cover', borderRadius: '0.25rem', border: '1px solid #e5e7eb' }} />
                    </div>
                )}
            </div>
        </div>
...
const RegistrationForm = () => {
    ...
    const handleParticipantChange = (index, event) => {
        ...
        if (type === 'file') {
            ...
            newParticipants[index]['proofFile'] = file || null;
            newParticipants[index]['proofUrl'] = file ? URL.createObjectURL(file) : '';
        } else {
            newParticipants[index][name] = type === 'checkbox' ? checked : event.target.value;
        }
        setParticipants(newParticipants);
    };
    ...
    const validateForm = () => {
        const errors = {};
        participants.forEach((p, i) => {
            if (!p.name) errors[`p${i}_name`] = 'Name is required';
            if (!p.designation) errors[`p${i}_designation`] = 'Designation is required';
            if (!p.organisation) errors[`p${i}_organisation`] = 'Organisation is required';
            if (!p.email) errors[`p${i}_email`] = 'Email is required';
            if (!p.phone) errors[`p${i}_phone`] = 'Phone is required';
            if (!p.gender) errors[`p${i}_gender`] = 'Gender is required';
            if (!p.proofFile) errors[`p${i}_proofFile`] = 'Proof upload is required';
        });
        ...
        if (!formData.abstractTitle) errors.abstractTitle = 'Abstract title is required';
        if (!formData.abstractContent) errors.abstractContent = 'Abstract content is required';
        if (wordCount > 300) errors.abstractContent = 'Abstract must not exceed 300 words';
        if (!formData.abstractExpression) errors.abstractExpression = 'Keywords are required';
        if (!genuineSubmission) errors.genuineSubmission = 'Please confirm the submission is genuine';
    ...
    const handleSubmit = async (event) => {
        ...
        // Append participant info as a JSON string and their files separately
        const participantInfo = participants.map(({ proofFile, proofUrl, ...rest }) => rest);
        submissionData.append('participants', JSON.stringify(participantInfo));
        
        participants.forEach((p, index) => {
            if (p.proofFile) {
                submissionData.append(`proofFile_${index}`, p.proofFile);
            }
        });
                ...
                <div className="right-panel">
                    ...
                    <form onSubmit={handleSubmit} className="form-body">
                        <fieldset className="form-fieldset">
                            ...
                            <button type="button" onClick={addParticipant} className="btn-add" disabled={participants.length >= 4}>+ Add Another Participant</button>
                        </fieldset>
                        ...
                        <fieldset className="form-fieldset">
                            ...
                            <div className="confirmation-group">
                                ...
                                <label htmlFor="genuineSubmission" className="form-label-sm">I declare that the information submitted is genuine.</label>
                            </div>
                            <div className="confirmation-group">
                                <input type="checkbox" id="termsAccepted" checked={termsAccepted} readOnly 
                                    onClick={() => setIsTermsModalOpen(true)} className="form-checkbox" />
                                <label htmlFor="termsAccepted" className="form-label-sm">I agree to the <span onClick={() => setIsTermsModalOpen(true)} className="terms-link">Terms and Conditions</span>.</label>
                            </div>
                        </fieldset>
                ...
                {isModalOpen && (
                    <div className="modal-overlay">
                        <div className="modal-content">
                            ...
                            <h3 className="modal-title">Registration Submitted!</h3>
                            <p className="modal-description">Thank you for registering. You can check your status on the status page.</p>
                            <pre className="modal-data-preview">{JSON.stringify(submittedData, null, 2)}</pre>
                        </div>
                    </div>
                )}
                {isTermsModalOpen && (
                    <TermsModal onAccept={handleAcceptTerms} onDecline={handleDeclineTerms} onClose={handleDeclineTerms} />
                )}
            </div>
        </React.Fragment>
    );
